...
public class ExplosiveCompactorMenu extends AbstractContainerMenu {
    ...
    public ExplosiveCompactorMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(7));
    }

    public ExplosiveCompactorMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(AtomicMenuTypes.EXPLOSIVE_COMPACTOR_MENU.get(), pContainerId);
        checkContainerSize(inv, 7);
        this.blockEntity = ((ExplosiveCompactorBlockEntity) entity);
