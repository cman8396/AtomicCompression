...
public class ExplosiveSmelterMenu extends AbstractContainerMenu {
    public final ExplosiveSmelterBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public ExplosiveSmelterMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(7));
    }

    public ExplosiveSmelterMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(AtomicMenuTypes.EXPLOSIVE_SMELTER_MENU.get(), pContainerId);
        checkContainerSize(inv, 6);
        this.blockEntity = ((ExplosiveSmelterBlockEntity) entity);
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, AtomicBlocks.EXPLOSIVE_SMELTER.get());
    }
