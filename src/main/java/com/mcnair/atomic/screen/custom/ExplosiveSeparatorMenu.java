...
public class ExplosiveSeparatorMenu extends AbstractContainerMenu {
    public final ExplosiveSeparatorBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public ExplosiveSeparatorMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(7));
    }

    public ExplosiveSeparatorMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(AtomicMenuTypes.EXPLOSIVE_SEPARATOR_MENU.get(), pContainerId);
        checkContainerSize(inv, 7);
        this.blockEntity = ((ExplosiveSeparatorBlockEntity) entity);
        ...
        // Output Secondary
        this.addSlot(new OutputOnlySlot(blockEntity.itemHandler, blockEntity.itemHandler::set, 5, 91, 60));
        // Output Tertiary
        this.addSlot(new OutputOnlySlot(blockEntity.itemHandler, blockEntity.itemHandler::set, 6, 111, 60));
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, AtomicBlocks.EXPLOSIVE_SEPARATOR.get());
    }
