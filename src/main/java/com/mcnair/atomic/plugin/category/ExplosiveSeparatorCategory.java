...
public class ExplosiveSeparatorCategory implements IRecipeCategory<RecipeHolder<ExplosiveSeparatorRecipe>> {
    public static final IRecipeHolderType<ExplosiveSeparatorRecipe> TYPE = IRecipeHolderType.create(ExplosiveSeparatorRecipe.Type.INSTANCE);

    private final IDrawable background;
    private final IDrawable icon;

    public ExplosiveSeparatorCategory(IGuiHelper helper) {
        Identifier texture = Identifier.fromNamespaceAndPath(AtomicCompression.MOD_ID, "textures/gui/jei.png");
        background = helper.createDrawable(texture, 0, 104, 152, 52);

        icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(AtomicBlocks.EXPLOSIVE_SEPARATOR.get()));
    }

    @Override
    public IRecipeHolderType<ExplosiveSeparatorRecipe> getRecipeType() {
        return TYPE;
    }
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder iRecipeLayoutBuilder, RecipeHolder<ExplosiveSeparatorRecipe> recipe, IFocusGroup iFocusGroup) {
        ...
        iRecipeLayoutBuilder.addSlot(RecipeIngredientRole.OUTPUT, 81, 32).
        ...
        iRecipeLayoutBuilder.addSlot(RecipeIngredientRole.OUTPUT, 101, 32).
                addItemStacks(outputEntries[2].isEmpty() ? List.of() : List.of(outputEntries[2])).
                addRichTooltipCallback((view, tooltip) ->
                        StandardOutputPercentages(recipe.value().getSecondaryOutput().percentages(), view, tooltip));
    ...
    @Override
    public void draw(RecipeHolder<ExplosiveSeparatorRecipe> recipe, IRecipeSlotsView iRecipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
